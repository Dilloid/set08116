...
bool render() {
  ...
  for (int i = 0; i < 2; i++) {
      ...
      // Set inverse width
      glUniform1f(blur.get_uniform_location("inverse_width"),
          (1.0f / temp_frames[i].get_width()));
      // Set inverse height Uniform
      glUniform1f(blur.get_uniform_location("inverse_height"),
          (1.0f / temp_frames[i].get_height()));
      // Render screen quad
